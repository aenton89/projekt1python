...
class Logic:
    ...
    # zmiana stanu fal na wodzie
    # TODO: napisac     
    def update_water_stage(self, x_mouse, y_mouse):
        self.frame_counter += 1
        if self.frame_counter >= FPS:
            self.frame_counter = 0

        self.gui.water_stage = True if self.frame_counter % FRAMES_PER_IMAGE_CHANGE < FRAMES_PER_IMAGE_CHANGE / 2 else False
        if(self.gui.water_stage == False):
            self.gui.water_stage = True
        else:
            self.gui.water_stage = False
